...
class PlateParameters(BaseModel):
    ...
    estimated_weight: float | None = Field(
        None,
        ge=0,
        description="Estimated filament weight in grams.",
        examples=[45.5],
    )
    estimated_time: int | None = Field(
        None,
        ge=0,
        description="Estimated printing time in seconds.",
        examples=[3600],
    )
...
class PlateUpdateParameters(BaseModel):
    ...
    @classmethod
    def prevent_none(cls: type["PlateUpdateParameters"], v: str | None) -> str | None:
...
@router.get(
    ...
    name="Find plate",
    description=(
        "Get a list of plates that matches the search query. "
        "A websocket is served on the same path to listen for updates to any plate, or added or deleted plates. "
...
async def find(
    ...
    project_id: Annotated[
        ...
        Query(
            title="Project ID",
            description="Filter plates by project ID.",
        ),
    ] = None,
    name: Annotated[
        str | None,
        Query(
            title="Plate Name",
            description=(
                "Partial case-insensitive search term for the plate name. Separate multiple terms with a comma. "
...
@router.get(
    ...
    name="Get plate",
    description=(
        "Get a specific plate. A websocket is served on the same path to listen for changes to the plate. "
        "See the HTTP Response code 299 for the content of the websocket messages."
    ),
    response_model_exclude_none=True,
