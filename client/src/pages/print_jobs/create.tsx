...
export const PrintJobCreate = (_props: IResourceComponentsProps) => {
  const [searchParams] = useSearchParams();
  const plateIdFromUrl = searchParams.get("plate_id") ? Number(searchParams.get("plate_id")) : undefined;

  const { form, formProps, formLoading, onFinish, redirect } = useForm<IPrintJob, HttpError, IPrintJob, IPrintJob>();

  const { selectProps: plateSelectProps } = useSelect<IPlate>({
    resource: "plate",
    optionLabel: "name",
    optionValue: "id",
    defaultValue: plateIdFromUrl,
  });
  ...
  if (plateIdFromUrl && !formProps.initialValues.plate_id) {
    formProps.initialValues.plate_id = plateIdFromUrl;
  }
  ...
  const handleSubmit = async (redirectTo: "list" | "edit" | "create") => {
    ...
    const payload = {
      ...values,
      start_time: values.start_time ? (values.start_time as unknown as dayjs.Dayjs).toISOString() : undefined,
      end_time: values.end_time ? (values.end_time as unknown as dayjs.Dayjs).toISOString() : undefined,
    };
    const result = await onFinish(payload);
    const id = (result as { data?: IPrintJob } | undefined)?.data?.id;
    redirect(redirectTo, id);
  };
